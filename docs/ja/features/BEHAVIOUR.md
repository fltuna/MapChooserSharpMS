...
- `!nomlist` はこの選出順をそのまま反映し、投票に入らないエントリ (枠超過または `MinNominationCountForVote` 未満) の上にセパレータ行を表示します。差し替えイベントによる変更は予測できないため反映されません。
- ノミネート数が利用可能なマップ枠 (`MaxMenuElements`。延長予算が残っている間は *Extend* オプション分 1 枠減) を超えている間は、ノミネート成立のたびに全ノミネーターへ「投票マップはノミネート数順に選出される」旨と `!unnom` での取り消し方法がチャット通知されます。
